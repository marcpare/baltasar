...
def roll_up_arguments(node):
  if node[0] == 'argument-list':
    ret = [parse(node[1])]
    ret.extend(roll_up_arguments(node[2]))
    return ret
  else:
    return [parse(node[1])]
...
def parse(node):
  ...
  if node[0] == 'binary-expression':
    ...
    arguments = roll_up_arguments(node[2])
    
    # for now, functions hard-coded
    if node[1] == 'sum':      
      domain = sets[node[2][2][1][1]]
      the_var = node[2][1][1]
      unrolled = []
      for x in domain:
...
if __name__ == "__main__":
  ...
  add_variable("a", "-oo", "+oo")
  add_variable("b", "-oo", "+oo")
  add_variable("c", "-oo", "+oo")
  print quick_constraint("a = b + c")
  print quick_constraint("a = b - c")
  print quick_constraint("a = b * c")
  print quick_constraint("a = b / c")
  print quick_constraint("a = -b")
  
  add_set("times", range(3))
  add_variable("d", "-oo", "+oo", range(3))
  print quick_constraint("d[t] = -b for t in times")
  
  print quick_constraint("sum(d, times) = 10.0")
  
  print quick_constraint("min(5, 10) < 3")
  print quick_constraint("min(d[t], 10+2) < 3 for t in times")